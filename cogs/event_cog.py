...
class EventCog(commands.Cog):
    ...
    @app_commands.describe(story_id="رقم القصة التي تود تشغيلها")
    @app_commands.describe(round_seconds="مدة التصويت لكل جولة بالثواني (افتراضي: 30)")
    @app_commands.default_permissions(manage_guild=True)
    @app_commands.checks.has_permissions(manage_guild=True)
    async def start_event(self, interaction: discord.Interaction, story_id: int, round_seconds: app_commands.Range[int, 15, 120] = 30):
        ...
        if not story:
            ...
            await interaction.followup.send(f"❌ لم يتم العثور على قصة برقم {story_id}.\nالقصص الجماعية المتاحة: {available_stories}", ephemeral=True)
            return

        if story.game_mode != "multi":
            await interaction.followup.send(f"❌ القصة برقم {story_id} ليست مخصصة للعب الجماعي. استخدم أمر `/لعب_فردي`.", ephemeral=True)
            return
        ...
        self.bot.loop.create_task(
            self.bot.event_manager.start_event(interaction.channel, story_id, voting_timeout=float(round_seconds))
        )
